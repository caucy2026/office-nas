...
若 native 配置失败，工作流会将完整 smoke 脚本输出、`engine/autogen.sh` 输出和 `engine/config.log` 中与错误相关的脱敏片段及末尾写入 Job Summary，同时保留完整日志 Artifact；无需把 GitHub Token 发到聊天中即可定位依赖问题。
